...
const Shipping = () => {
    ...
    const Districts = pcVN.getDistrictsByProvinceCode(getProvinces.province_code);
    const Wards = pcVN.getWards();

    
    const onSelectProvince = (provId) => {
        const selDi = Districts.filter(c => c.province_code === provId);
        this.setState({
        provinceId: provId,
        Districts: selDi,
    });
    ...
    const onSelectDistrict = (dist) => {
        this.setState({
            distId: dist.district_code
        });
      }
